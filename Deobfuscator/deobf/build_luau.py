...
def main():
    ...
    found = []
    for name in (exe, ast_exe):
        for cand in (os.path.join(build, "Release", name), os.path.join(build, name)):
            ...
            sys.exit("[!] built binary not found under " + build + ": " + name)
    if len(found) != 2:
        sys.exit("[!] Luau runtime build incomplete")
    if tmp:
